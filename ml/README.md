...
| **第四版** | **ViT-B-32 / clip-b32（512）** | **1756→1753（good579/bad1174）** | **0.825** | **0.823** | **0.724** | **0.751** | **0.737** |
...
- 第四版线上模型（开 dedup 重训）混淆矩阵：TP=435 TN=1008 FP=166 FN=144，误判 310 张。随机CV(0.825)≈分组CV(0.823)，无泄漏，数字真实。
- 第三/四版换数据集后分数不可直接横比；当前 good:bad ≈ 1:2、**FP 偏多（精确率 0.72，模型说 good 的约 1/4 实为 bad）**，下一步重心是**补难负样本压 FP**，而非继续调 backbone。
...
- [ ] **【当前重心】补难负样本压 FP**：当前精确率仅 0.72（模型说 good 的约 1/4 实为 bad），针对性补"看着像 good 但其实 bad"的图，靠闭环纠错收紧边界
- [ ] 继续转闭环，每轮纠错提升精度

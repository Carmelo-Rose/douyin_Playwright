...
| 版本 | 样本(去重后) | 随机CV acc | 分组CV acc（真实） | 精确率(good) | 召回率(good) | F1 |
|---|---|---|---|---|---|---|
| 第一版 | 245（good124/bad121） | 0.82 | — | 0.812 | 0.839 | — |
| 第二版 | 1005（good266/bad739） | 0.894 | — | 0.787 | 0.820 | — |
| **第三版** | **1924→1910（good401/bad1509）** | **0.945** | **0.944** | **0.852** | **0.888** | **0.869** |

特征：open_clip ViT-B-32, 512 维。第三版经过两轮主动学习闭环，新增919张纠错标注（dedup删除14张近似重复）。
混淆矩阵：TP=356 TN=1447 FP=62 FN=45。随机CV与分组CV差值仅+0.001，无明显数据泄漏。

...
async function main(): Promise<void> {
  ...
  try {
    ...
    const videos = filterByMaxAgeDays(dedupeVideos(allVideos), config.maxAgeDays);

    if (videos.length === 0) {
      console.warn("No video records were recognized after source capture and time filtering. Try DEBUG_CAPTURE=true to inspect XHR URLs and dumped JSON in output/debug-*.json.");
      return;
    }
...
async function captureJingxuan(page: Page, capture: NetworkCaptureHandle, config: RuntimeConfig): Promise<VideoRecord[]> {
...
async function captureRootSearch(page: Page, capture: NetworkCaptureHandle, config: RuntimeConfig): Promise<VideoRecord[]> {
...
async function collectByScrolling(page: Page, config: RuntimeConfig): Promise<void> {
...
async function waitIfCaptcha(page: Page): Promise<void> {
  ...
  console.log("⚠️  检测到滑块/验证码，请在浏览器中手动完成验证，脚本会自动继续...");
